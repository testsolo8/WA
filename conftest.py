...
from lang_locators import (
    MainNavigateButtons,
    ReportPageSoftwareOnComputers,
    ReportPageSystemReports,
    ReportPageProgramControllerReports,
)
...
# @fixture(scope="function", params=["chromium"])
@fixture(scope="function", params=["chromium", "webkit", "firefox"], ids=["chromium", "webkit", "firefox"])
...
@pytest.fixture(scope="session")
def report_navigate_buttons_software_on_computers(lang_parametrize):
    return ReportPageSoftwareOnComputers(lang_parametrize)


@pytest.fixture(scope="session")
def report_navigate_buttons_system_reports(lang_parametrize):
    return ReportPageSystemReports(lang_parametrize)


@pytest.fixture(scope="session")
def report_navigate_buttons_program_controller_reports(lang_parametrize):
    return ReportPageProgramControllerReports(lang_parametrize)
...
def pytest_addoption(parser):
    parser.addoption("--base_url", help="base url of site under test", default="https://localhost")
    parser.addini("headless", help="run browser in headless mode", default="True")
    parser.addini("slowmo", help="run browser in slowmo mode", default=00)
